...
def create_app() -> Dash:
    ...
    init_overlay_val = init_overlay_days[-1] if init_overlay_days else None
    if today_str in init_overlay_days and len(init_overlay_days) > 1:
        if len(store.load_prices(init_sym, today_str)) < 5:
            init_overlay_val = init_overlay_days[-2]
    ...
    if init_overlay_val:
        ...
        if init_chain is not None and init_spot is not None:
            ...
            init_overlay_fig = build_options_overlay(
                init_sym, init_chain, init_spot, init_flip, init_res["keys"], init_overlay_val, 0.03, snapshots=init_snaps
            )
    ...
    def overlay_days(symbol, current):
        ...
        default_day = days[-1]
        if today in days and len(days) > 1:
            today_prices = store.load_prices(symbol, today)
            if len(today_prices) < 5:
                default_day = days[-2]
        return options, current if current in days else default_day
    ...
    def refresh_options_overlay(
        ...
        if not day:
            days = available_overlay_days(symbol)
            default_day = days[-1] if days else today
            if today in days and len(days) > 1 and len(store.load_prices(symbol, today)) < 5:
                default_day = days[-2]
            day = default_day
        ...
        if chain is None or not current_spot:
            return build_options_overlay(symbol, None, None, None, None, day, window,
        ...
        fig.update_layout(uirevision=f"overlay-{symbol}-{day}-{bucket}-{timeframe or '2d'}")
        return fig
    ...
    def refresh_heatmap(_, tab, heat_sub, symbol, window, lang, unit, day, levels_shown, metric, cfd_offset,
        ...
        if heat_sub == "intraday":
            return (cards,
                    build_intraday_heatmap(symbol, lang, day, window, xf, unit, levels_shown, metric),
                    no_update, no_update, no_update, no_update, t(lang, "heat_hint"))
        if heat_sub == "bubbles":
            return (cards, no_update,
                    heatmap_bubbles_fig(symbol, lang, day, window, xf, unit, levels_shown, metric),
